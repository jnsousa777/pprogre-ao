...
export async function loadCloudConfig() {
  ...
  if (window.PROGRESSAO_CONFIG?.supabaseUrl && window.PROGRESSAO_CONFIG?.supabaseKey) return window.PROGRESSAO_CONFIG;
  try {
    const response = await fetch("/api/config", { cache: "no-store" });
    if (response.ok) {
      const config = await response.json();
      if (config.supabaseUrl && config.supabaseKey) return config;
    }
...
export function saveCloudConfig(config) {
  ...
  if (!/^https:\/\/.+\.supabase\.co$/i.test(normalized.supabaseUrl)) throw new Error("Cole a URL do projeto Supabase, terminando em .supabase.co.");
  if (normalized.supabaseKey.length < 20) throw new Error("A chave publicável parece incompleta.");
...
export function initializeCloud(config) {
  ...
  client = supabaseGlobal().createClient(config.supabaseUrl, config.supabaseKey, {
